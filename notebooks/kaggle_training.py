# ============================================================
# GraphSense — Kaggle Training Notebook
# Optimal config: threshold=0.1, alpha=0.0 (classify only)
#
...
# Verify normalization worked (check AFTER applying scaler)
...
# ============================================================
# CELL 7 — Train (optimal config: thresh=0.1, alpha=0.0)
# ============================================================
import wandb
...
print("=" * 55)
print("TEST SET RESULTS — GraphSense GAT (thresh=0.1, α=0.0)")
print("=" * 55)
...
# ROC + PR curves
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle("GraphSense GAT — Test Set Curves", fontsize=13)
...
X_train = all_node_feats[train_idx]   # already normalized via scaler above
...
# Re-normalize X_train/X_test using the same scaler
...
print(f"{'ROC-AUC':<12} {roc_auc_score(y_test, iso_scores):>12.4f} {gat_auc:>12.4f}")
print(f"{'F1':<12} {f1_score(y_test, iso_preds):>12.4f} {gat_f1:>12.4f}")
...
def train_and_eval(alpha, threshold, epochs=30):
    ...
    tr_l = DataLoader([ds[i] for i in train_idx], batch_size=BATCH_SIZE, shuffle=True)
    ...
    preds = (np.array(probs_all) > 0.35).astype(int)
